#!/usr/bin/env python3
"""voice-31 as recorded. No pitch/formant conversion. Dub only."""
...
TAKES = ROOT / "dubs/0911-1/clone/real-dub/from-full-srt/v31"
VIDEO = ROOT / "library/0911-1/source.mp4"
OUT = HERE / "final-dub-v31-raw.mp4"
SR = 24000
PAUSE = 0.50
...
def main() -> int:
    ...
    t = 0.0
    for i in range(4):
        take, _ = trim_silence(decode(TAKES / f"seg-{i:04d}.mp3"), SR)
        if i:
            parts.append(gap)
            t += PAUSE
        start = t
    ...
    subprocess.run(
        [ffmpeg_exe(), "-y", "-v", "error", "-i", str(wav), "-c:a", "libmp3lame", "-b:a", "160k", str(HERE / "final-dub-v31-raw.mp3")],
        check=True,
        capture_output=True,
    )
    ...
    isl = speech_islands(out_a, SR, floor_db=-34.0, min_dur=0.20, merge_gap=0.40)
    print("dur", round(dur, 3), "corr", round(c, 4), "lagged", round(lc, 4), "lag", round(lag, 3), "streams", n_audio, "islands", len(isl))
    if n_audio != 1:
    ...
    (HERE / "final-dub-v31-raw.srt").write_text(srt, encoding="utf-8")
    meta = {
        "voice_id": "voice-31",
        "processing": "none",
        "tempo": 1.0,
    ...
    (HERE / "final-dub-v31-raw.agent.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
