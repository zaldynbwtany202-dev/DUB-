#!/usr/bin/env python3
"""voice-30 dub only. Natural speed. No original narrator in the file."""
...
def main() -> int:
    ...
    subprocess.run(
        [ffmpeg_exe(), "-y", "-v", "error", "-i", str(wav), "-c:a", "libmp3lame", "-b:a", "160k", str(HERE / "final-dub-v30.mp3")],
        check=True,
        capture_output=True,
    )
    ...
    (HERE / "final-dub-v30.srt").write_text(srt, encoding="utf-8")
    meta = {
        "voice_id": "voice-30",
        "tempo": 1.0,
    ...
    (HERE / "final-dub-v30.agent.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
