#!/usr/bin/env python3
"""voice-31 retake, mux only. No DSP."""
...
OUT = HERE / "final-dub-v31-match.mp4"
SR = 24000
PAUSE = 0.50
...
def main() -> int:
    ...
    subprocess.run(
        [ffmpeg_exe(), "-y", "-v", "error", "-i", str(wav), "-c:a", "libmp3lame", "-b:a", "160k", str(HERE / "final-dub-v31-match.mp3")],
        check=True,
        capture_output=True,
    )
    ...
    (HERE / "final-dub-v31-match.srt").write_text(srt, encoding="utf-8")
    meta = {
        "voice_id": "voice-31",
        "processing": "none",
        "tempo": 1.0,
    ...
    (HERE / "final-dub-v31-match.agent.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
