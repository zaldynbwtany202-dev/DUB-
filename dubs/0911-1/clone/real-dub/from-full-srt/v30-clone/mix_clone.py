#!/usr/bin/env python3
"""Clone original-narrator timbre onto the approved voice-30 takes. Dub only."""
...
TAKES = ROOT / "dubs/0911-1/clone/real-dub/from-full-srt/v30"
VIDEO = ROOT / "library/0911-1/source.mp4"
OUT = HERE / "final-dub-v30-clone.mp4"
SR = 24000
PAUSE = 0.50
MAX_SEMITONES = 1.8
MAX_FORMANT = 0.12
DRY = 0.32  # keep some of the human TTS consonants
...
def main() -> int:
    ...
    subprocess.run(
        [ffmpeg_exe(), "-y", "-v", "error", "-i", str(wav), "-c:a", "libmp3lame", "-b:a", "160k", str(HERE / "final-dub-v30-clone.mp3")],
        check=True,
        capture_output=True,
    )
    ...
    (HERE / "final-dub-v30-clone.srt").write_text(srt, encoding="utf-8")
    meta = {
        "voice_id": "voice-30-clone-original",
        ...
        "conversions": reports,
        "target_f0": None if target is None else target.f0_median,
        "merge": "forbidden",
        "output": str(OUT.relative_to(ROOT)),
    }
    if target is not None:
        meta["target_f0"] = round(target.f0_median, 1)
        meta["target_f1"] = round(target.f1, 1)
    (HERE / "final-dub-v30-clone.agent.json").write_text(
